...
public class dijkstra <T> implements GraphADT<T> {
	...
	protected class Path implements Comparable<Path> {
		...
		public Path(Path copyPath, Edge extendBy) {
			...
			this.dataSequence = new LinkedList<>();
			copyPath.dataSequence.forEach(x -> {
	...
	protected Path dijkstrasShortestPath(T start, T end) {
		...
		if(!containsVertex(start) || !containsVertex(end)) {
			throw new NoSuchElementException("Could not find a path between the two given nodes");
		}
		...
		// Create initial path
		Path newPath = new Path(begin);

		// Add initial path to priority queue
		discoveredPaths.add(newPath);

		while (!discoveredPaths.isEmpty()) {
			// Get lowest cost path to next node
			Path path = discoveredPaths.remove();
			// Add that node to visited
			visitedNodes.add(path.end);

			// If node that is visited is the node we are looking for return the path
			if (path.end.data.equals(end)) {
				return path;
			}

			// Iterate through edges of current node
			for (Edge edge : path.end.edgesLeaving) {
				Vertex endNode = edge.target;

				/*
				 * If next node has not been visited, the node is in queue, check for shorter
				 * path.
				 * 
				 * If the node has not been visited, the node is not in queue, add the node to
				 * queue
				 * 
				 * If the node has been visited, do nothing
				 */
				if (!visitedNodes.contains(endNode)) {
					Path currPath = new Path(path, edge);
					// If the node is in queue and there is a shorter path, update the old path to
					// the new path
					if (checkForPathInQueue(discoveredPaths, endNode, currPath)) {
						if (checkForShorterPath(discoveredPaths, endNode, currPath)) {
							PriorityQueue<Path> tempQueue = new PriorityQueue<Path>();
							for (int x = 0; x < discoveredPaths.size(); x++) {
								if (!discoveredPaths.peek().end.equals(endNode)) {
									tempQueue.add(discoveredPaths.remove());
								} else {
									discoveredPaths.remove();
								}
							}
							discoveredPaths = tempQueue;
							discoveredPaths.add(currPath);
						}
					}
					// If the node is not in queue
					else {
						discoveredPaths.add(currPath);
					}
				}
	...
	/**
	 * Checks if given path is in the priority queue of paths
	 * 
	 * @param discovered paths queue of paths ordered by cost
	 * @param endNode    last node of given path
	 * @param currPath   new found path for a node
	 * @return True if node is in the queue
	 */
	private boolean checkForPathInQueue(PriorityQueue<Path> discoveredPaths, Vertex endNode, Path currPath) {
		for (Path oldPath : discoveredPaths) {
			if (oldPath.end.equals(endNode)) {
				return true;
			}
		}
		return false;
	...
	/**
	 * Checks if a shorter path exists for a given node
	 * 
	 * @param discoveredPaths Queue of all paths
	 * @param endNode         Node that multiple paths exist for
	 * @param currPath        A new path found for a node
	 * @return True if new path is shorter than old path
	 */
	private boolean checkForShorterPath(PriorityQueue<Path> discoveredPaths, Vertex endNode, Path currPath) {
		for (Path oldPath : discoveredPaths) {
			if (oldPath.end.equals(endNode)) {
				if (oldPath.compareTo(currPath) > 0)
					return true;
			}
		}
		return false;
