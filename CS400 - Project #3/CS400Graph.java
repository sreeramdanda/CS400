...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
	    ...
	    // FIXME Potentially need to fix this
    ...
    protected Path dijkstrasShortestPath(T start, T end) {        
	    if (start == null || end == null)
		    throw new NoSuchElementException("Cannot start at or end at a null element");
	   // Hashtable is named vertices
	   // create priority queue
	   // pq = priority queue
           PriorityQueue<Path> pq = new PriorityQueue<>();
	   // create visited hashtable
	   Hashtable<Vertex, Boolean> visitedVertices = new Hashtable<>();
	   // add start to queue
	   Vertex startVertex = vertices.get(start);
	   if (startVertex == null)
		   throw new NoSuchElementException("No such start vertex exists: " + startVertex);
	   pq.add(new Path(startVertex));
	   Path shortestPath = null;
	   while (!pq.isEmpty()) {
		   Path path = pq.poll();
		   // Compares the current shortest path to the one pulled if it has the same start and end
		   if (path.start.data.equals(start) && path.end.data.equals(end) && (shortestPath == null || path.compareTo(shortestPath) < 0))
			   shortestPath = path;
		   // add all new Paths from current path to the queue
		   LinkedList<Edge> edges = path.end.edgesLeaving;
		   for (Edge e : edges) {
			   // Add all unvisited vertices
			   if (!visitedVertices.containsKey(e.target)) {
		   		visitedVertices.put(path.end, true);
				pq.add(new Path(path, e));
			   }
		   }
	   }
	   if (shortestPath != null)
		   return shortestPath;
	   throw new NoSuchElementException("Could not find a path from " + start + " to " + end);
    }
